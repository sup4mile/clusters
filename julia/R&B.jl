...
@NLexpression(m, lv_ic_H_calc[1:ni, 1:nc],
    yv_ic_H[i,j]*((z_H[i,j]*L_ic_H[i,j]^η)^(-1)))
@NLexpression(m, lv_ic_Hx_calc[1:ni, 1:nc],
    yv_ic_Hx[i,j]*((z_H[i,j]*L_ic_H[i,j]^η)^(-1)))
@NLexpression(m, lv_ic_F_calc[1:ni],
    τ * yv_ic_F[i,j]*((z_F[i])^(-1)))
@NLexpression(m, lv_ic_F_calc[1:ni],
    yv_ic_Fx[i]*((z_F[i])^(-1)))
