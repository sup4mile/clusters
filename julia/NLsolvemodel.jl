...
function yv_if_F(i,l)
        i = trunc(Int, i)
        result = pv_if_F(i,l)^(-ρ) * p_i_H(i,l)^(ρ-σ) * p_H(l)^(σ-1) * E_H
        if !isfinite(result)
            println("Oops, this value is not finite: $(pv_if_F(i,l))^$(-ρ) * $(p_i_H(i,l))^$(ρ-σ) * $(p_H(l))^$(σ-1) * $E_H")
        end
...
function f!(F,l)
    ...
    for i in 1:ni
        ...
        for j in 1:nc
            temp = yv_ic_H(i,j,l) / (z_H[i,j] * L_ic_H(i,j,l) ^ η) - l[i,j]
            if isfinite(temp)
                F[i,j] = temp
            else
                println("Oops, this value is not finite: $yv_ic_H(i,j,l) / ($z_H[i,j] * $L_ic_H(i,j,l) ^ $η) - $l[i,j]")
            end
        end

        # fixed point for lv_ic_Hx
        for j in nc+1:2nc
            temp = yv_ic_Hx(i,j-nc,l)/ (z_H[i,j-nc] * L_ic_H(i,j-nc,l) ^ η) - l[i,j]
            if isfinite(temp)
                F[i,j] = temp
            else
                println("Oops, this value is not finite: $yv_ic_Hx(i,j-nc,l)/ ($z_H[i,j-nc] * $L_ic_H(i,j-nc,l) ^ $η) - $l[i,j])")
            end
        end

        # fixed point for lv_if_F
        # lv_if_F starts at col = 2nc+1
        temp = τ * yv_if_F(i,l) / z_F[i] - l[i,2nc+1]
        if isfinite(temp)
            F[i, 2nc+1] = temp
        else
            println("Oops, this value is not finite: $τ * $(yv_if_F(i,l)) / $z_F[i] - $l[i,2nc+1]")
        end

        # fixed point for lv_if_Fx
        # lv_if_Fx starts at col = 2nc+2
        temp = τ * yv_if_Fx(i,l) / z_F[i] - l[i,2nc+2]
        if isfinite(temp)
            F[i, 2nc+2] = temp
        else
            println("Oops, this value is not finite: $τ * $yv_if_Fx(i,l) / $z_F[i] - $l[i,2nc+2]")
        end
    ...
    if isfinite(temp)
        ...
        println("Oops, this value is not finite: ex($l) - imp($l)")
    end

    temp = l[1,2nc+3] - l[2, 2nc+3]
    if isfinite(temp)
        F[2, 2nc+3] = temp
    else
        println("Oops, this value is not finite: $l[1,2nc+3] - $l[2, 2nc+3]")
    end
