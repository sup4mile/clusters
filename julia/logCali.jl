...
lv_ic_H = [0.5696 0.5696; 0.222647 0.222647; 0.0748392 0.0748392]
# lv_ic_H = [0.5696 0.5696; 0.222647 0.222647; 0 0]
lv_ic_Hx = [0.0873127 0.0873127;  0.0341291  0.0341291;0.0114719 0.0114719]
lv_if_F = [0.200617; 0.284265; 0.382204]
lv_if_Fx = [0.030752; 0.043574; 0.058587]
w_F = 0.153288
...
##############################################################################################################
    # """
    #     force domestic others labor to be zero, avoid negativity
    # """
    # for i in 1:ni-1
    #     for j in 1:nc
    #         F[i,j] = (yv_ic_H(i,j,l)) / (l[i,2nc+4] * L_ic_H(i,j,l) ^ η) - l[i,j]
    ...
    #
    # for j in 1:nc
    #     F[3,j] =(yv_ic_H(3,j,l)) / (l[3,2nc+4] * L_ic_H(3,j,l) ^ η)-0.0001
    # end
    #
    # for j in nc+1:2nc
    #     F[3,j] = (yv_ic_Hx(3,j-nc,l)) / (l[3,2nc+4] * L_ic_H(3,j-nc,l) ^ η)-0.0001
    # end
##############################################################################################################
    # trade balance condition
    F[1,2nc+3] = ex(l) - imp(l)

    # sectorial import share target
    F[1, 2nc+4] = import_ratio(1,l) - target_H[1]
    F[2, 2nc+4] = import_ratio(2,l) - target_H[2]
    # z_H[3] others productivity at home normalized
    F[3,2nc+4] = l[3,2nc+4] -2.1
...
result = nlsolve(f!, l_initial, autodiff =:forward, iterations = 100000, method =:newton, xtol = 1e-5, ftol = 1e-5)
result.zero
